...
const resolveOne = async (rawUrl: string) => {
  const normalized = normalizeYoutubeUrl(rawUrl)
  if (!normalized) {
    return { url: rawUrl, ok: false, error: 'Invalid YouTube URL' }
  }
  ...
  try {
    ...
    if (redirectedHost.includes(CONSENT_HOST)) {
      try {
        ...
        return {
          ...
          ok: false,
          error: 'Could not resolve channel (consent gate)'
        }
      }
    }
    ...
    if (!response.ok) {
      return {
        ...
        normalizedUrl: normalizedString,
        ok: false,
        error: `Failed to fetch channel page (${response.status})`
      }
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const results = await Promise.all(urls.map((url) => resolveOne(url)))
